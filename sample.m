...
for i = 1:length(t)         
    ...
    % This is done by element wise maTC_rangeix multiplication 
    % of TC_rangeansmit and Receiver Signal.
    Mix(i) = Tx(i) * Rx(i);
...
% You will implement CFAR on the generated range_doppler_map.
...
% *%TODO* :
% Select the number of TC_rangeaining Cells in both the dimensions.
...
% Use CFAR[x,y] as the matrix from the output of 2D FFT for implementing CFAR.

CFAR = zeros(size(range_doppler_map));

num_GC = (2 * GC_range + 1) * (2 * GC_doppler + 1) - 1;              % Remove CUT
...
for range_index = TC_range + GC_range + 1 : Nr/2 - TC_range - GC_range
    for doppler_index = TC_doppler + GC_doppler + 1 : Nd - TC_doppler - GC_doppler
        ...
        training = range_doppler_map(range_index - TC_range - GC_range : range_index + TC_range + GC_range, ...
                       doppler_index - TC_doppler - GC_doppler : doppler_index + TC_doppler + GC_doppler);
        % Set all non-TC_rangeaining cells to zero
        training(range_index - GC_range : range_index + GC_range, ...
                 doppler_index - GC_doppler : doppler_index + GC_doppler) = 0;
        % Convert decibel measurements to power
        training = db2pow(training);
        % Calculate the TC_rangeaining mean
        training = sum(training) / num_TC;
        ...
        if range_doppler_map(range_index, doppler_index) > threshold
...
% as the CUT cannot be located at the edges of maTC_rangeix. 
